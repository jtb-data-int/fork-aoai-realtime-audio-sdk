...
// 環境変数からデフォルト値を設定
const envAzureEndpoint = import.meta.env.VITE_AZURE_OPENAI_ENDPOINT || '';
const envAzureApiKey = import.meta.env.VITE_AZURE_OPENAI_API_KEY || '';
const envAzureDeployment = import.meta.env.VITE_AZURE_OPENAI_DEPLOYMENT || '';
...
const defaultSystemMessage = `あなたは経験豊富な旅行プランナーです。質問者の趣味や志向に応じて最適なプランを提示することができます。
なお、会話に当たっては以下を守るようにしてください。
・話し終わったら、質問者が応答するまで勝手に話さないでください。会話のキャッチボールを意識してください
・わからないことについては、はっきり「わかりません」と伝えるようにしてください`;

// 環境変数から値を初期設定
formEndpointField.value = envAzureEndpoint;
formApiKeyField.value = envAzureApiKey || envOpenAIApiKey;
formDeploymentOrModelField.value = envAzureDeployment || envOpenAIModel;
formAzureToggle.checked = !!envAzureEndpoint;
formSessionInstructionsField.value = defaultSystemMessage;
